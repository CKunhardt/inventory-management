<template>
  <div class="reports">
    ...
    <div v-else-if="quarterlyData.length === 0 && monthlyData.length === 0" class="empty-state">
      {{ t('common.noData') }}
    </div>
    <div v-else>
  ...
  setup() {
    ...
    const bestQuarter = computed(() => {
      return quarterlyData.value.reduce((best, q) => {
        return q.total_revenue > best.revenue ? { quarter: q.quarter, revenue: q.total_revenue } : best
      }, { quarter: '', revenue: 0 }).quarter
    })
    ...
    const formatQuarter = (quarter) => {
      ...
      return t('reports.quarterFormat', { number: match[1], year: match[2] })
    }
    ...
    const formatMonth = (monthStr) => {
      ...
      return t('reports.monthFormat', { month: t(`months.${monthKeys[monthIndex]}`), year: match[1] })
    }
